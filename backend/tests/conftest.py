...
"""Application-wide test fixtures and configuration.

This module provides test-suite-wide fixtures and configuration.
As a reminder, any test sub-package can include it's own conftest.py which can
either add new sub-package specific fixtures or, when necessary, override fixtures
defined "at a higher level" e.g. here.

Additionally, the test_*.py files can always declare their own fixtures if the use
of the fixture is limited to a single test module.
"""
...
@pytest.fixture()
def sample_fixture() -> Literal[True]:
    """Placeholder fixture available to all tests.

    This is part of the repo template, and can be deleted once used.
    """
    return True
